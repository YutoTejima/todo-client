...
async function addTask() {
  ...
  const response = await fetch(requestUrl, {
    ...
    body: JSON.stringify({
      ...
      completedAt: taskForm.completedAt || undefined,
    }),
  });
...
<template>
  <div :class="$style.container">
    ...
    <form :class="$style.form">
      ...
      <button type="button" :class="[$style.button, $style.primary]" @click="addTask" :disabled="status === 'pending'">
        追加
      </button>

      <button type="button" :class="[$style.button, $style.secondary]" @click="clearTask">クリア</button>

      <p v-if="status === 'success'" :class="$style.successMessage">タスクを作成しました</p>
      <p v-if="status === 'error'" :class="$style.errorMessage">タスクの作成に失敗しました</p>
    </form>
  </div>
</template>
